...
export function FiltersBar({ q, year, session, years, onPatch, onClear }) {
  return (
    <div className="mt-3 flex flex-wrap items-center justify-between gap-2.5 border-[6px] border-win95 bg-win95 p-3 shadow-[6px_6px_0_#000] [border-style:ridge]">
      <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2.5">
        ...
        <Select value={session} onValueChange={(v) => onPatch({ session: v })}>
...
export function TechTags({ active, onToggle }) {
  return (
...
export function ZipBlock({ exams }) {
  const withZip = exams.filter((e) => e.zipZalaczniki);
  return (
