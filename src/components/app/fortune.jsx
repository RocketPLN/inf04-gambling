...
import { THEORY_CATEGORIES, THEORY_QUESTIONS } from "@/data/theory.js";
...
export function FortuneWheel({ onLanded }) {
...
export function TheoryQuestionCard({ question, onResult, total }) {
      ...
      <CardContent className="p-0">
        ...
        {revealed && (
          <div className={`mt-5 border-[4px] p-3 text-sm font-black sm:text-base ${correct ? "border-cke-green bg-green-100 text-black" : "border-ugly-red bg-ugly-yellow text-black"} [border-style:inset]`}>
            ...
            <div className="mt-2 border-t-2 border-dashed border-current pt-2 font-bold">💡 {question.wyjasnienie}</div>
          </div>
        )}
      </CardContent>
    </Card>
...
export function ScoreBar({ score, streak, answered }) {
