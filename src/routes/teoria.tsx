...
function TheoryPage() {
  ...
  const handleResult = (ok: boolean) => {
    ...
    if (ok) {
      ...
      if (count("streak-freeze") > 0 && consume("streak-freeze")) {
        setFlash("[*] STREAK FREEZE ZADZIAŁAŁ!!! SERIA URATOWANA, 1 SZTUKA ZUŻYTA!!!");
      } else {
        setStreak(0);
      }
  ...
  const takeDogrywka = () => {
    ...
    setDogrywkaOffer(false);
    setHalfArmed(true);
    setFlash("[R] DOGRYWKA!!! Następne pytanie z tej kategorii za PÓŁ nagrody. Honor do uratowania!!!");
  ...
  useEffect(() => {
    ...
    if (examLeft <= 0) {
      setExamLeft(null);
      setFlash(`[...] KONIEC EGZAMINU!!! Wynik sesji: +${examEarned} pkt w 10 minut. CKE kiwa głową (może).`);
      setExamEarned(0);
      return;
    }
    ...
    <>
      ...
      <div className="mt-3.5 border-[6px] border-ugly-red bg-gradient-to-br from-ugly-pink via-ugly-yellow to-ugly-cyan p-4 shadow-[8px_8px_0_#000] [border-style:ridge]">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          ...
          {examModeOwned && !examRunning && (
            <Button variant="slot" size="sm" onClick={() => { setExamLeft(EXAM_SECONDS); setExamEarned(0); setFlash("[...] EGZAMIN RUSZYŁ!!! 10 minut, wynik ukryty. Powodzenia, zdawaczu!!!"); }}>
              [...] START EGZAMINU (10:00) [...]
            </Button>
          )}
          <Button variant="slot" size="sm" asChild>
            <Link to="/sklep" search={DEFAULT_SEARCH}>{'[>>] SKLEP ARCADE'}</Link>
          </Button>
          <Button variant="claim" size="sm" asChild>
            <Link to="/rogue" search={DEFAULT_SEARCH}>[D] LOCH ROGUE-LIKE</Link>
          </Button>
      ...
      <div className="mt-3.5">
        {examRunning ? (
          <div className="flex flex-wrap items-center gap-2 border-[5px] border-ugly-red bg-black p-2.5 font-mono text-[11px] font-black text-ugly-yellow shadow-[5px_5px_0_#000] [border-style:ridge]">
            ...
            <button
              onClick={() => { setExamLeft(null); setExamEarned(0); setFlash("[...] Egzamin przerwany. CKE udaje, że nie widziało."); }}
      ...
      {dogrywkaOffer && category && (
        <div className="mt-3.5 border-[5px] border-casino-gold bg-black p-3 text-center shadow-[5px_5px_0_#000] [border-style:ridge]">
          <div className="font-display text-lg font-black uppercase text-casino-gold">[R] WTOPA... ALE JEST DOGRYWKA!!! [R]</div>
